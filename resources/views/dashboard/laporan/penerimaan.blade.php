@extends('layouts.app')
@section('title', 'Laporan Penerimaan Bahan Baku')
...
<div class="container py-5">
    <div class="card rounded-4 px-3 py-4 p-sm-3 p-md-4 p-lg-5 bg-white">
        ...
        <div class="d-flex flex-column flex-sm-row justify-content-between align-items-center gap-3 mb-3 card-header-theme pb-3">
            <h3 class="fw-bold mb-0">📥 Laporan Penerimaan Bahan (Terbanyak)</h3>
        ...
        <div class="card rounded-3 border-0 shadow-sm">
            <div class="card-body p-0 table-responsive rounded-3">
                <table class="table table-striped table-borderless mb-0 rounded-3">
                    ...
                    <tbody>
                        @forelse($bahanPalingMasuk as $index => $item)
                        <tr class="bg-white">
                            <td class="align-middle fw-bold">{{ $bahanPalingMasuk->firstItem() + $index }}</td>
                            <td class="align-middle">{{ $item->bahanBaku->nama ?? 'N/A' }}</td>
                            <td class="align-middle">{{ number_format($item->total_masuk, 0, ',', '.') }} {{ $item->bahanBaku->satuan_label ?? '' }}</td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="3" class="text-center py-5">
                                <p class="text-muted">Tidak ada data penerimaan pada periode yang dipilih.</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
        ...
        <div class="d-flex justify-content-end mt-3">
            {{ $bahanPalingMasuk->appends(request()->query())->links() }}
        </div>
    </div>
</div>
@endsection